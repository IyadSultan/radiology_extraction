...
class AdditionalFindingItem(BaseModel):
    """Model for additional findings with defined properties"""
    name: str = Field(description="Name or identifier of finding")
    description: str = Field(description="Description of the additional finding")

class LesionDetail(BaseModel):
    """Model for lesion details"""
    location: str = Field(description="Anatomical location")
    size: Optional[str] = Field(default=None)
    characteristics: Optional[str] = Field(default=None)

class TumorDetail(BaseModel):
    """Model for tumor details"""
    size: str = Field(description="Size of tumor")
    location: str = Field(description="location of tumor")
    shape: Optional[str] = Field(default=None)
    margins: Optional[str] = Field(default=None)
    density: Optional[str] = Field(default=None)
    internal_structure: Optional[str] = Field(default=None)
    
class Edema(BaseModel):
    """Model for edema details"""
    description: str = Field(description="Description of the edema")
    location: str = Field(description="Location of the edema")
    
class MassEffect(BaseModel):
    """Model for Mass Effect Details"""
    description: str = Field(description="Description of the mass effect")
    location: str = Field(description="Location of the mass effect")
...
class SUVMeasurement(BaseModel):
    ...
    suv_mean: Optional[float] = Field(default=None)
    suv_peak: Optional[float] = Field(default=None)
    prior_suv_max: Optional[float] = Field(default=None)
    percent_change: Optional[float] = Field(default=None)
    metabolic_volume: Optional[float] = Field(default=None)
    measurement_time: Optional[str] = Field(default=None)
...
class PETCTFindings(BaseModel):
    ...
    injection_time: Optional[str] = Field(default=None)
    blood_glucose: Optional[float] = Field(default=None)
    uptake_time: Optional[str] = Field(default=None)
    suv_measurements: List[SUVMeasurement] = Field(default_factory=list)
    total_lesion_glycolysis: Optional[float] = Field(default=None)
    background_suv: Optional[float] = Field(default=None)
    metabolic_response: Optional[str] = Field(default=None)
    uptake_pattern: Optional[str] = Field(default=None)
...
class MammographyFindings(BaseModel):
    ...
    masses: List[LesionDetail] = Field(default_factory=list)
    calcifications: List[LesionDetail] = Field(default_factory=list)
    architectural_distortion: Optional[bool] = Field(default=None)
    asymmetries: List[LesionDetail] = Field(default_factory=list)

class ChestCTFungalFindings(BaseModel):
    """Chest CT findings focused on fungal infections"""
    halo_sign: List[LesionDetail] = Field(default_factory=list)
    cavitations: List[LesionDetail] = Field(default_factory=list)
    fungal_nodules: List[LesionDetail] = Field(default_factory=list)
    ground_glass_opacities: List[LesionDetail] = Field(default_factory=list)
    air_crescent_signs: List[LesionDetail] = Field(default_factory=list)
    other_fungal_findings: List[AdditionalFindingItem] = Field(default_factory=list)

class BrainTumorFindings(BaseModel):
    """Brain imaging findings focused on tumors"""
    tumor_details: TumorDetail = Field(...)
    edema: Optional[Edema] = Field(default=None)
    mass_effect: Optional[MassEffect] = Field(default=None)
    enhancement_pattern: Optional[str] = Field(default=None)
    brain_region: str = Field(description="Specific brain region affected")
    additional_features: List[AdditionalFindingItem] = Field(default_factory=list)
...
class ModalitySpecific(BaseModel):
    ...
    mammography: Optional[MammographyFindings] = Field(default=None)
    chest_ct: Optional[ChestCTFungalFindings] = Field(default=None)
    brain_tumor: Optional[BrainTumorFindings] = Field(default=None)
    pet_ct: Optional[PETCTFindings] = Field(default=None)
...
class RECISTMeasurement(BaseModel):
    ...
    def standardize_measurements(self):
        """Standardize the measurements to mm"""
...
class RadiologyReport(BaseModel):
    ...
    normal_study: Optional[bool] = Field(default=None)
    ...
    new_lesions: List[LesionDetail] = Field(default_factory=list)
    ...
    modality_specific: Optional[ModalitySpecific] = Field(default=None)
...
class ModalityMapper(EnhancedSimilarityMapper):
    ...
    def find_closest_modality(self, text: str, threshold: float = 0.3) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            elif any(brain_term in text_lower for brain_term in ['brain', 'head', 'cranial']) and any(modal_term in text_lower for modal_term in ['ct', 'mri', 'magnetic']):
...
standard_extraction_agent = Agent(
    "gemini:gemini-pro-flash",  # Changed to Gemini 2.0 Flash
    retries=3,
...
class EnhancedRadiologyExtractor:
    ...
    def __init__(self, modalities_df: pd.DataFrame, topography_df: pd.DataFrame):
        """Initializes the extractor with modality and location mappers."""
...
async def main():
    ...
    try:
        ...
        output_file = Path('example_output_gemini.json')
        output_data = {
